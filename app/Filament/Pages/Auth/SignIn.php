...
class SignIn extends Login
{
  ...
  {
    return TextInput::make('card_number')
      ->label('No KK / Email')
      ->required()
      ->autocomplete()
      ->autofocus();
  ...
  public function authenticate(): ?LoginResponse
  {
    ...
    $find_profile = Profile::where('card_number', $data['card_number'])->first();
    ...
    if (!$find_profile) {
      ...
      if (!Filament::auth()->attempt($this->getCredentialsFromFormData($credentials), $data['remember'] ?? false)) {
        $this->throwFailureValidationException();
      }
